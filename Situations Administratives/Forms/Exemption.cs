...
namespace Situations_Administratives.Forms
{
    public partial class Exemption : Form
    {
       ...
       string.IsNullOrEmpty(TxtObser.Text))
            {
            ...
            try
            {
                ...
                string query = "INSERT INTO الاعفاء ([الاسم], [رقم_التأجير], [الاطار], [الفترة_الزمنية], [ابتداء_من], [المؤسسة], [ملاحظات]) " +
                               "VALUES (@name, @number, @comb, @dat, @lic, @eco, @obs)";
            ...
            new SqlParameter("@obs", TxtObs)
                ...
                if (result > 0)
                {
                    ...
                    ShowExe(); // Méthode pour afficher les données après insertion
                    BtnVider_Click(this, EventArgs.Empty); // Méthode pour vider les champs
                }
                else
                {
       ...
       string.IsNullOrEmpty(TxtObser.Text))
            ...
            try
            {
                ...
                // Requête SQL avec paramètres pour la table 'الاعفاء'
                string query = "UPDATE الاعفاء SET رقم_التأجير = @number, الاطار = @comb, " +
                               "الفترة_الزمنية = @dat, ابتداء_من = @lic, المؤسسة = @eco, ملاحظات = @obs " +
                               "WHERE الاسم = @name";
            ...
            new SqlParameter("@obs", TxtObs)
                ...
                if (result > 0)
                {
                    ...
                    ShowExe(); // Actualiser les données
        ...
        {
            if (key == 0)  // Assurez-vous que 'key' est l'ID sélectionné
            {
            ...
            else
            {
                ...
                new SqlParameter("@key", key)
                    ...
                    if (rowsAffected > 0)
                    {
                        ...
                        ShowExe(); // Actualiser l'affichage des données
                        BtnVider_Click(this, EventArgs.Empty); // Vider les champs
                    }
                    else
                    {
        ...
        private void btnImp_Click(object sender, EventArgs e)
        {
            ...
            AjusterColonnesPourPage(DGVExe, printer);
        ...
        private void btnExport_Click(object sender, EventArgs e)
        {
            if (DGVExe.Rows.Count > 0)
            {
                ...
                try
                {
                    ...
                    for (int i = 1; i < DGVExe.Columns.Count + 1; i++)
                    {
                        Excel.Range headerCell = worksheet.Cells[startRow, i];
                        headerCell.Value2 = DGVExe.Columns[i - 1].HeaderText;
                    ...
                    for (int i = 0; i < DGVExe.Rows.Count; i++)
                    {
                        for (int j = 0; j < DGVExe.Columns.Count; j++)
                        {
                            var cellValue = DGVExe.Rows[i].Cells[j].Value;
                    ...
                    using (SaveFileDialog saveFileDialog = new SaveFileDialog())
                    {
                        ...
                        saveFileDialog.FileName = "الاعفاء من مهام التدريس.xlsx";
        ...
        private void btnRecherche_Click(object sender, EventArgs e)
        {
            ...
            new SqlParameter("@nom", TxtSearch.Text.Trim())
                ...
                {
                    DGVExe.DataSource = dataTable; // Afficher les résultats dans le DataGridView
