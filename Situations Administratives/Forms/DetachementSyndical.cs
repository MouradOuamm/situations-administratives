    ...
    public partial class DetachementSyndical : Form
    {
        ...
        private void btnAjouter_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                string query = "INSERT INTO [التفرغ_النقابي] ([الاسم], [رقم_التاجير], [الإطار], [الفترة_الزمنية], [ابتداء_من], [الهيئة_النقابية]) " +
        ...
        TxtCombo.SelectedIndex == -1 || string.IsNullOrEmpty(TxtApartir.Text))
            ...
            try
            {
                ...
                // Requête SQL avec paramètres
                string query = "UPDATE التفرغ_النقابي SET رقم_التاجير = @number, الإطار = @comb, " +
                               "الفترة_الزمنية = @dat, ابتداء_من = @lic, الهيئة_النقابية = @eco " +
                               "WHERE الاسم = @name";
        ...
        private void btnImp_Click(object sender, EventArgs e)
        {
            ...
            AjusterColonnesPourPage(DGVSyndical, printer);
        ...
        private void btnExport_Click(object sender, EventArgs e)
        {
            if (DGVSyndical.Rows.Count > 0)
            {
                ...
                try
                {
                    ...
                    for (int i = 1; i < DGVSyndical.Columns.Count + 1; i++)
                    {
                        Excel.Range headerCell = worksheet.Cells[startRow, i];
                        headerCell.Value2 = DGVSyndical.Columns[i - 1].HeaderText;
                    ...
                    for (int i = 0; i < DGVSyndical.Rows.Count; i++)
                    {
                        for (int j = 0; j < DGVSyndical.Columns.Count; j++)
                        {
                            var cellValue = DGVSyndical.Rows[i].Cells[j].Value;
                    ...
                    using (SaveFileDialog saveFileDialog = new SaveFileDialog())
                    {
                        ...
                        saveFileDialog.FileName = " التفرغ النقابي.xlsx";
